<!-- src/views/report/PlatformRevenueReport.vue -->
<template>
  <div class="platform-revenue-page">
    <!-- Header + Summary -->
    <MCard>
      <div class="pr-page-head">
        <div class="pr-page-head__main">
          ...
          <p class="pr-page-head__sub">
            查看指定期間內的平台營收、訂單表現與整體營運統計資料。
          </p>
        </div>

        <div class="pr-page-head__actions">
          <span class="pr-current-type">平台全站統計</span>

          <span v-if="hasReportData" class="pr-total-count">
            共 {{ totalRowCount }} 筆資料
          </span>
        </div>
      </div>
      ...
      <div class="pr-summary-row">
        <div class="pr-summary-card">
          <span class="pr-summary-card__label">查詢範圍</span>
          <strong class="pr-summary-card__value">平台全站</strong>
        </div>

        <div class="pr-summary-card">
          <span class="pr-summary-card__label">查詢期間</span>
          <strong class="pr-summary-card__value">
            {{ queryDateText }}
          </strong>
        </div>

        <div class="pr-summary-card">
          <span class="pr-summary-card__label">明細資料</span>
          <strong class="pr-summary-card__value">
            {{ totalRowCount }} 筆
          </strong>
        </div>
    ...
    <div class="m-t-12">
      <MCard>
        <div class="pr-card-head">
          <div>
            <p class="pr-card-head__title">查詢條件</p>
            <p class="pr-card-head__sub">可依日期區間查詢平台營收統計資料。</p>
          </div>
        </div>

        <Form ref="formRef" :initial-values="initValues" @submit="onSubmit">
          <div class="pr-filter-grid">
            <div class="pr-filter-grid__date">
              <FormDateRangeField
                label="查詢日期"
                type="date"
    ...
    <div class="m-t-12">
      <MCard>
        <div class="pr-card-head pr-card-head--result">
          <div>
            <p class="pr-card-head__title">查詢結果</p>
            <p class="pr-card-head__sub">
              依目前查詢條件顯示平台營收摘要、圖表與明細資料。
            </p>
          </div>

          <div class="pr-result-actions">
            <span v-if="hasReportData" class="pr-card-head__count">
              共 {{ totalRowCount }} 筆資料
            </span>
        ...
        <div v-if="forbiddenMessage" class="pr-forbidden m-t-16">
          {{ forbiddenMessage }}
        </div>

        <div v-else-if="loading" class="pr-state m-t-16">查詢中...</div>
        ...
        <template v-else>
          ...
          <div v-else class="m-t-16">
            <!-- 統計卡片 -->
            <div v-if="summaryEntries.length" class="pr-stat-row">
              <div
                v-for="entry in summaryEntries"
                :key="entry.key"
                class="pr-stat-card"
              >
                <span class="pr-stat-card__label">{{ entry.label }}</span>
                <strong class="pr-stat-card__value">
                  {{ entry.value }}
                </strong>
              </div>
            </div>
            ...
            <!-- 圖表 -->
            <div v-if="chartOption" class="pr-chart m-t-20">
              <div class="pr-chart__head">
                <div>
                  <p class="pr-chart__title">平台營收趨勢圖</p>
                  <p class="pr-chart__sub">依目前資料呈現平台營收數據變化。</p>
                </div>
              </div>

              <VChart :option="chartOption" class="pr-chart__main" autoresize />
            </div>
            ...
            >
              <div class="pr-section-head">
                <p class="pr-section-head__title">{{ section.title }}</p>

                <span class="pr-section-head__count">
...
const chartOption = computed(() => {
  ...
  const categoryField =
    'date' in firstRow
      ? 'date'
      : (['storeName', 'lotteryTitle', 'name', 'title'].find(
          (key) => key in firstRow,
...
async function onSubmit(values: any) {
  ...
  await executeApi({
    fn: async () => {
      ...
      return getPlatformRevenueReport(req);
    },
    onSuccess: (res: any) => {
