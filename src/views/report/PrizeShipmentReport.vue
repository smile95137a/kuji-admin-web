<!-- src/views/report/PrizeShipmentReport.vue -->
<template>
  <div class="prize-shipment-page">
    ...
    <MCard>
      <div class="ps-page-head">
        <div class="ps-page-head__main">
          ...
          <p class="ps-page-head__sub">
            查看指定期間內的獎品出貨統計、出貨狀態與明細資料。
          </p>
        </div>

        <div class="ps-page-head__actions">
          <span class="ps-current-type">獎品出貨統計</span>
      ...
      <div class="ps-summary-row">
        ...
        <div class="ps-summary-card">
          <span class="ps-summary-card__label">摘要欄位</span>
          <strong class="ps-summary-card__value">
        ...
        <div class="ps-summary-card">
          <span class="ps-summary-card__label">明細資料</span>
          <strong class="ps-summary-card__value">
            {{ totalRowCount }} 筆
          </strong>
        </div>
    ...
    <div class="m-t-12">
      <MCard>
        ...
        <div v-else-if="loading" class="ps-state m-t-16">查詢中...</div>
          ...
          <div v-else class="m-t-16">
            ...
            <div v-if="chartOption" class="ps-chart m-t-20">
              <div class="ps-chart__head">
                <div>
                  <p class="ps-chart__title">出貨資料分布圖</p>
                  <p class="ps-chart__sub">依目前明細資料呈現數量分布。</p>
                </div>
              </div>
            ...
            >
              <div class="ps-section-head">
                <p class="ps-section-head__title">{{ section.title }}</p>
...
const chartOption = computed(() => {
  ...
  const categoryField =
    ['status', 'shipmentStatus', 'prizeName', 'storeName', 'date'].find(
      (key) => key in firstRow,
  ...
  if (!categoryField || !numericFields.length) return null;
  ...
  return {
    ...
    series: [
      {
        name: toReportLabel(firstNumericField),
        type: 'bar',
        data: rows.map((row: any) => row[firstNumericField] ?? 0),
        barMaxWidth: 44,
      },
    ],
  };
...
async function onSubmit(values: any) {
  ...
  await executeApi({
    fn: async () => {
      ...
      return getPrizeShipmentReport(req);
    },
    onSuccess: (res: any) => {
