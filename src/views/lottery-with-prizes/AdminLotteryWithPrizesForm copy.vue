<!-- src/views/lottery-with-prizes/AdminLotteryWithPrizesForm copy.vue -->
<template>
  <div class="lotteryWithPrizesForm">
    ...
    <template v-if="isEdit">
      ...
      <MCard
        v-if="
          gameMode === 'SCRATCH_STORE' && lotteryDesignationStatus === 'PENDING'
        "
      ...
      >
        ...
        >
          <span style="color: #d46b08; font-size: 13px">
            ⚠️
            此刮刮樂商品（店家指定模式）尚未完成大獎號碼指定。開始抽獎前需先指定大獎號碼。
          </span>
          <MButton size="sm" @click="showDesignateModal = true"
            >前往指定大獎號碼</MButton
          >
        </div>
      </MCard>
      ...
      <MCard
        v-if="
          ...
          lotteryDesignationStatus === 'DESIGNATED'
        "
      ...
      >
        <div style="padding: 12px 16px; color: #005a99; font-size: 13px">
          ℹ️
          此刮刮樂商品（玩家指定模式）：玩家購票後自行指定大獎號碼，無需店家操作。
        </div>
      </MCard>
    </template>
...
const onSubmit = handleSubmit(async (values) => {
  ...
  if (
    ...
    await openInfoDialog({
      title: '無法儲存',
      message:
        '此刮刮樂商品（店家指定模式）尚未完成大獎號碼指定，無法設為抽獎中，請先完成指定流程。',
      iconType: 'warning',
    });
    return;
  }
  ...
  try {
    const payload = {
      lottery: {
        ...
        delistStrategy: cleanText((values as any).delistStrategy) || undefined,
        paymentType: cleanText((values as any).paymentType) || undefined,
        freeDrawThreshold:
